...
Now you should access CLI script: $ my-fastapi-project --help
Alternatively you can access CLI script via poetry run: $ poetry run my-fastapi-project --help
To deactivate virtualenv simply type: $ deactivate
To activate shell completion:
 - for bash: $ echo 'eval "$(_MY_FASTAPI_PROJECT_COMPLETE=source_bash my-fastapi-project)' >> ~/.bashrc
 - for zsh: $ echo 'eval "$(_MY_FASTAPI_PROJECT_COMPLETE=source_zsh my-fastapi-project)' >> ~/.zshrc
 - for fish: $ echo 'eval "$(_MY_FASTAPI_PROJECT_COMPLETE=source_fish my-fastapi-project)' >> ~/.config/fish/completions/my-fastapi-project.fish
EOF